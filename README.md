...
> **Forwarded headers are sanitized, not relayed.** A client's own `X-Forwarded-Host`, `X-Forwarded-Proto`, and `X-Forwarded-For` are discarded before the request reaches your back-end; they're rebuilt from values this proxy vouches for (the configured [`host`](#host), when set, plus the real connection's protocol and IP). This closes the classic attack where a spoofed `X-Forwarded-Host` is used to poison password-reset links or OAuth redirects generated by a back-end that blindly trusts that header. If this instance itself sits behind another trusted proxy or load balancer, see [`trustProxy`](#trustproxy).
...
Express's [`trust proxy`](https://expressjs.com/en/guide/behind-proxies.html) setting for this port. Leave it unset unless this instance itself runs **behind another trusted reverse proxy or load balancer** (see [Behind a reverse proxy](#behind-a-reverse-proxy)) — the default (`false`, nothing trusted) is what makes header spoofing described above impossible.
...
**No `ssl` config needed** in `server-config.json` when the outer proxy handles TLS. Set [`trustProxy: 1`](#trustproxy) in `server-config.json` so this proxy honors the `X-Forwarded-*` headers the outer proxy sets (below) instead of ignoring them.
